...
- **NFR-12 — Security:** strict CSP with per-request nonces (`script-src 'self' 'nonce-…' 'strict-dynamic'`), `frame-ancestors 'none'`, HSTS 2 years with preload, `Permissions-Policy: microphone=(), camera=(), geolocation=()`. Form endpoints rate-limited to 5 requests / 10 min / IP via Upstash Redis (v1.0 specified Vercel KV, which Vercel has since retired).
